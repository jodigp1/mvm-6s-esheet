...
export default function DashboardPage() {
  ...
  const [sortRanking, setSortRanking] = useState<'avg' | 'nama' | 'total'>('avg')
  ...
  // ── Ranking filtered & sorted ─────────────────────────────────
  const ranking = useMemo(() => {
    if (!data) return []
    let list = [...data.ranking]
    if (filterKat !== 'ALL') list = list.filter(r => r.dominanKat === filterKat)
    if (sortRanking === 'avg')   list.sort((a, b) => b.avgPct - a.avgPct)
    if (sortRanking === 'nama')  list.sort((a, b) => a.nama.localeCompare(b.nama))
    if (sortRanking === 'total') list.sort((a, b) => b.totalAudit - a.totalAudit)
    return list
  }, [data, filterKat, sortRanking])
  ...
  async function exportLokasiCSV() {
    if (!data) return
    const rows = [
      ...
      <header className="bg-white border-b border-surface-border sticky top-0 z-50 shadow-card">
        <div className="max-w-2xl mx-auto px-5 h-[60px] flex items-center gap-3">
          ...
          <div className="flex-1">
            <div className="text-[15px] font-extrabold text-ink">Dashboard Analitik</div>
            <div className="text-[11px] text-ink-3">6S Audit — {tahun}</div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-5 py-6 flex flex-col gap-5">
        ...
        <div className="card fade-up">
          ...
          <div className="p-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div>
              <label className="text-[11px] font-bold text-ink-2 mb-1 block">Tahun</label>
              <select value={tahun} onChange={e => setTahun(e.target.value)} className="inp text-xs py-2">
                {YEARS.map(y => <option key={y} value={y}>{y}</option>)}
              </select>
            </div>
            <div>
              <label className="text-[11px] font-bold text-ink-2 mb-1 block">Lokasi</label>
              <select value={lokasiId} onChange={e => setLokasiId(e.target.value)} className="inp text-xs py-2">
                <option value="ALL">Semua</option>
                {lokasiList.map(l => <option key={l.id} value={l.id}>{l.nama}</option>)}
              </select>
            </div>
            <div>
              <label className="text-[11px] font-bold text-ink-2 mb-1 block">Semester</label>
              <select value={semester} onChange={e => setSemester(e.target.value)} className="inp text-xs py-2">
                <option value="ALL">Semua</option>
            ...
            <div>
              <label className="text-[11px] font-bold text-ink-2 mb-1 block">Kategori</label>
              <select value={filterKat} onChange={e => setFilterKat(e.target.value)} className="inp text-xs py-2">
                <option value="ALL">Semua</option>
        ...
        {!loading && data && (
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 fade-up">
              {[
                { label: 'Total Sesi',    value: data.kpi.totalSesi,    icon: 'assignment',   color: 'text-brand',   bg: 'bg-brand-pale' },
                { label: 'Auditee Dinilai', value: data.kpi.totalAuditee, icon: 'people',     color: 'text-brand',   bg: 'bg-brand-pale' },
                { label: 'Avg Score',     value: `${data.kpi.avgScore}%`, icon: 'percent',    color: 'text-success', bg: 'bg-success-light' },
                { label: 'Di-skip',       value: data.kpi.totalSkipped, icon: 'person_off',   color: 'text-warning', bg: 'bg-warning-light' },
                ...
                <div className="card-head flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-[11px] text-ink-3">EX · SD · NI</div>
                    </div>
                  </div>
                </div>
                <div className="p-4">
                ...
                <div className="card-head flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-[11px] text-ink-3">Avg score & distribusi</div>
                    </div>
                  </div>
              ...
              <div className="card-head flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] text-ink-3">Jumlah auditee per periode</div>
                  </div>
                </div>
              ...
              <div className="card-head flex items-center justify-between flex-wrap gap-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] text-ink-3">{ranking.length} auditee</div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <select value={sortRanking} onChange={e => setSortRanking(e.target.value as any)}
                    className="inp text-[11px] py-1.5 w-auto pr-7">
                    <option value="avg">Sort: Avg %</option>
                    <option value="nama">Sort: Nama</option>
                    <option value="total">Sort: Total Audit</option>
                  </select>
                </div>
              </div>
              ...
              <div className="card fade-up">
                <div className="card-head flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] text-ink-3">{data.skippedList.length} entri</div>
                  </div>
                </div>
